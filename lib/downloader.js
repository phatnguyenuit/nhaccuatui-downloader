const fs = require('fs');
const path = require('path');

const fetch = require('node-fetch');
const { parseStringPromise } = require('xml2js');

const SAMPLE_URL = "https://www.nhaccuatui.com/playlist/bai-hat-yeu-thich-thang-052021-va.sw52jWGuJEeT.html";

const getTracks = (nhaccuatuiYUrl) => new Promise((resolve, reject) => {
  fetch(nhaccuatuiYUrl)
    .then(response => response.text())
    .then(htmlText => {
      const regex = /player\.peConfig\.xmlURL\s=\s"(?<xmlUrl>.+)";/;
      const match = regex.exec(htmlText);
      const xmlUrl = match.groups['xmlUrl'];

      if (xmlUrl) {
        fetch(xmlUrl)
          .then(response => response.text())
          .then(xmlText => {
            return parseStringPromise(xmlText);
          })
          .then(result => {
            const tracks = result.tracklist.track || [];
            resolve(tracks.map(track => {
              const title = track.title[0].trim();
              const url = track.location[0].trim();
              const fileNameReg = /(?<fileName>[\w-\d]+\.mp3)/;
              const match = fileNameReg.exec(url);
              const fileName = match.groups.fileName || `${title}.mp3`;
              return {
                fileName,
                title,
                url
              };
            }));
          }).catch(e => reject(new Error('Failed to parse xml playlist.')));
      } else {
        reject(new Error('Failed to extract xml playlist location.'));
      }
    }).catch(e => reject(new Error('Failed to get playlist html content.')));
});

async function exists(path) {
  try {
    await fs.promises.access(path);
    return true;
  } catch {
    return false;
  }
}

const download = async (track, folderPath) => {
  try {
    const absolutePath = path.resolve(folderPath);
    const isExisted = await exists(absolutePath);
    if (!isExisted) {
      await new Promise((resolve, reject) => {
        fs.mkdir(absolutePath, { recursive: true }, (err, path) => {
          if (!err) {
            resolve();
          } else {
            reject(new Error(`Failed to create directory ${path}`));
          }
        });
      });
    }

    const res = await fetch(track.url);
    const fileStream = fs.createWriteStream(path.resolve(absolutePath, track.fileName));
    console.info(`Downloading file ${track.fileName}...`);
    await new Promise((resolve, reject) => {
      res.body.pipe(fileStream);
      res.body.on("error", reject);
      fileStream.on("finish", () => {
        console.info(`Downloaded file ${track.fileName}!`);
        resolve();
      });
    });

    return true;
  } catch (error) {
    throw new Error('Fail to download.');
  }
};

const runPromiseSequentially = (iterator, promiseCreator, ...params) => {
  return iterator.reduce(
    (prevPromise, item) =>
      prevPromise.then(_ => promiseCreator.apply(null, [item, ...params])),
    Promise.resolve()
  );
};

const downloader = async () => {
  try {
    const [url, folderPath, ..._] = process.argv.slice(2);
    console.info(`Prepare to download playlist from ${JSON.stringify(url)} to ${JSON.stringify(folderPath)}`);

    const tracks = await getTracks(url);
    await runPromiseSequentially(tracks, download, folderPath);
  } catch (error) {
    console.error(error.message);
  }
};

module.exports = downloader;
